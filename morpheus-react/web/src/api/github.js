...
// 커밋 목록 조회
export const getCommits = async (projectId, options = {}) => {
  const { limit, offset } = options;
  let url = `${API_URL}/commits/${projectId}`;
  const params = [];
...
// 브랜치 목록 조회
export const getBranches = async (projectId, options = {}) => {
  const { limit, offset } = options;
  let url = `${API_URL}/branches/${projectId}`;
  const params = [];
  if (limit) params.push(`limit=${limit}`);
  if (offset) params.push(`offset=${offset}`);
  if (params.length > 0) url += `?${params.join("&")}`;
