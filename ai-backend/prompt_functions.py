...
- **인프라 기능**: 핵심 기능에 속하지 않으므로 별도로 나열하세요 (coreFeatureId 없이).
...
### 4단계: 핵심 기능별 진행도 계산
- 각 핵심 기능에 대해:
  - 필요한 세부 기능 수 (2단계에서 정의한 것) - **정확히 세기**
  - 구현된 세부 기능 수 (실제로 확인된 것) - **정확히 세기**
...
- 예: "사용자 인증" 핵심 기능의 경우
  ...
  - 진행도: (4 / 5) × 100 = 80%
- **인프라 기능은 핵심 기능에 포함하지 마세요.** (별도로 나열)
...
- **인프라 기능은 핵심 기능에 포함하지 마세요.** (별도로 나열, coreFeatureId 없이)
...
- 인프라는 데이터베이스, 인증, 파일 업로드 등 백엔드 인프라 기능만 나열하세요. (핵심 기능 아님)
- **테스트/배포**: 테스트 파일이나 CI/CD 설정 파일을 확인하세요. 프로젝트 특성에 따라 없을 수 있습니다.
- **핵심 기능별 진행도 계산 검증**: 각 핵심 기능의 requiredCount와 implementedCount가 정확한지 확인하세요.
