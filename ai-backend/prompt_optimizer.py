...
9. **narrativeResponse**: **매우 중요** - 다음 **정확한 형식**으로 **마크다운 형식**의 상세한 설명 작성 (최소 2000자 이상):
   ...
   **반드시 다음 순서와 형식으로 작성하세요:**
   
   ## 1. 근거 (프로젝트 파악)
   읽은 파일(README, 설정 파일 등)과 프로젝트 설명을 바탕으로:
   - "이 프로젝트는 **[구체적인 프로젝트 유형/목적]** 프로젝트입니다."
   - 프로젝트의 목적, 기술 스택, 주요 특징을 구체적으로 설명
   
   ## 2. 필요한 요소들 (필수 기능/모듈)
   읽은 파일(README, package.json, 소스코드 구조 등)을 바탕으로 이 프로젝트에 있어야 할 주요 기능, 모듈, 컴포넌트를 **구체적으로** 나열:
   - **기능 1**: [기능명] - [왜 필요한지, 어떤 역할을 하는지 구체적 설명]
   - **기능 2**: [기능명] - [구체적 설명]
   - **기능 3**: [기능명] - [구체적 설명]
   - ... (최소 8개 이상, 읽은 파일에서 파악한 프로젝트 구조를 바탕으로)
   
   ⚠️ 각 기능은 읽은 파일 내용을 바탕으로 구체적으로 나열하세요. 예를 들어:
   - "사용자 인증 기능" (useAuth.js에서 확인됨)
   - "프로젝트 CRUD API" (projectController.js에서 확인됨)
   - "AI 챗봇 인터페이스" (ChatBot.jsx에서 확인됨)
   ...
   - ✅ **[기능/모듈명]**: 
     ...
     - 예: ✅ **사용자 인증 Hook**: 파일: `hooks/useAuth.js`, 함수: `useAuth()`, 구현 내용: JWT 토큰 관리 및 로그인 상태 확인 기능 구현됨
   - ✅ **[기능/모듈명]**: [동일한 형식으로]
   - ... (읽은 파일에서 실제로 확인된 것만 나열, 최소 5개 이상)
   
   ⚠️ 읽은 파일 내용을 무시하지 말고, 실제로 파일에서 확인된 기능만 나열하세요.
   
   ## 4. 개발되지 않은 부분 (소스코드 확인 결과)
   읽은 파일을 확인했지만 구현되지 않은 기능/모듈을 나열:
   - ❌ **[기능/모듈명]**: 
     - 필요한 이유: [왜 필요한지]
     - 예상 파일 위치: [어떤 파일에 있어야 하는지]
     - 예: ❌ **사용자 프로필 수정 기능**: 필요한 이유: 사용자가 자신의 정보를 수정할 수 있어야 함, 예상 파일 위치: `components/user/ProfileEdit.jsx` 또는 `controllers/userController.js`의 `updateProfile` 함수
   - ❌ **[기능/모듈명]**: [동일한 형식으로]
   - ... (필요한 요소 목록과 대조하여 누락된 기능 나열)
   
   ## 5. 진행도 계산 (매우 중요 - 이 값이 currentProgress와 일치해야 함)
   위 분석을 바탕으로 **논리적으로** 진행도를 계산:
   
   **계산 과정:**
   1. 필요한 요소 수: 총 **[N]**개 (섹션 2에서 나열한 요소 수)
   2. 개발된 요소 수: **[M]**개 (섹션 3에서 나열한 요소 수, 읽은 파일에서 실제로 확인된 것만)
   3. 개발되지 않은 요소 수: **[K]**개 (섹션 4에서 나열한 요소 수)
   4. 기본 진행도 계산: **[M] / [N] × 100 = [계산값]%**
   5. 검증: [M] + [K] = [N] (일치해야 함)
   
   **최종 진행도:**
   - 기본 진행도: **[계산값]%** (소수점 첫째 자리 반올림)
   - Task 완료율 참고: [Task 완료율]% (참고용)
   - 커밋 활동 참고: [커밋 활동 평가] (참고용)
   - **최종 진행도: [최종값]%** (기본 진행도를 우선시하되, Task/커밋 데이터가 크게 다르면 ±5% 조정 가능)
   
   ⚠️ **매우 중요**: 이 섹션에서 계산한 최종 진행도 값이 JSON의 **currentProgress** 필드와 **정확히 일치**해야 합니다.
