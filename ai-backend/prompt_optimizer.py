...
9. **narrativeResponse**: **매우 중요** - 다음 내용을 포함한 **마크다운 형식**의 상세한 설명 (최소 1000자 이상):
   
   **필수 포함 내용:**
   - **프로젝트 개요**: README 파일이나 프로젝트 설명을 분석하여 이 프로젝트가 무엇인지 설명
   - **필요한 요소들**: 이 프로젝트에 있어야 할 주요 기능, 모듈, 컴포넌트 등을 나열
   - **소스코드 분석**: 실제 소스코드를 확인하여 다음을 분석:
     * ✅ **완성된 부분**: 어떤 기능/모듈이 잘 구현되어 있는지 구체적으로 설명
     * ⚠️ **미완성 부분**: 어떤 기능/모듈이 부분적으로만 구현되었거나 불완전한지 설명
     * ❌ **누락된 부분**: 어떤 기능/모듈이 아직 구현되지 않았고 만들어야 하는지 설명
   - **프로젝트 목표**: 이 프로젝트의 최종 목표와 비전
   - **현재 구현 상태**: 지금까지 완료된 기능, 구현된 모듈, 달성한 마일스톤을 구체적으로 설명
...
- **narrativeResponse** 필드는 **마크다운 형식**으로 작성하세요 (제목, 리스트, 강조 등 사용).
- **최소 1000자 이상의 상세한 설명을 작성하세요. 500자 미만이면 분석이 불완전한 것으로 간주합니다.**
- 소스코드를 직접 확인하여 구체적인 파일명, 함수명, 모듈명을 언급하세요.
- README 파일이나 주요 설정 파일을 먼저 읽어서 프로젝트 구조를 파악하세요.
- 각 섹션(프로젝트 개요, 필요한 요소들, 소스코드 분석 등)을 모두 포함하여 작성하세요.
