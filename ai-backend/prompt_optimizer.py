...
{json.dumps(recent_commits_detail, ensure_ascii=False, indent=2)[:2000] if recent_commits_detail else '커밋 없음'}

## 열린 이슈 상세 (최근 {len(open_issues_detail)}개):
{json.dumps(open_issues_detail, ensure_ascii=False, indent=2)[:1500] if open_issues_detail else '이슈 없음'}
...
## 2차 분석: 실제 코드 분석 및 최종 판단

⚠️ **중요**: 반드시 Task 제목 "{task_title}"와 설명 "{task_description}"만 분석하세요. 다른 Task나 다른 기능은 완전히 무시하세요.

**실제 코드 파일을 읽어서 분석하세요.** 커밋 메시지만으로 판단하지 말고, 아래에 제공된 실제 파일 내용을 기반으로 판단하세요.
...
2. **기능 구현 여부 판단**:
   - 실제 코드 파일에서 Task 제목 "{task_title}"의 요구사항이 **완전히 구현**되어 있는가? → 완료
   - 실제 코드 파일에서 Task 제목 "{task_title}"의 요구사항이 **부분적으로 구현**되어 있는가? → 진행 중
