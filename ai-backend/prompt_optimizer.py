...
9. **narrativeResponse**: **매우 중요** - 다음 **정확한 형식**으로 **마크다운 형식**의 상세한 설명 작성 (최소 1500자 이상):
   ...
   **반드시 다음 순서와 형식으로 작성하세요:**
   ...
   ## 2. 필요한 요소들 (필수 기능/모듈)
   이 프로젝트에 있어야 할 주요 기능, 모듈, 컴포넌트를 나열:
   - 기능 1: [구체적 설명]
   - 기능 2: [구체적 설명]
   - 기능 3: [구체적 설명]
   - ... (최소 5개 이상)
   ...
   ## 3. 개발된 부분 (소스코드 확인 결과)
   실제 소스코드를 확인하여 구현된 기능/모듈을 구체적으로 나열:
   - ✅ **[기능/모듈명]**: [어떤 파일에서 확인됨, 어떤 함수/클래스로 구현됨]
   - ✅ **[기능/모듈명]**: [구체적인 구현 내용]
   - ... (파일명, 함수명, 클래스명을 구체적으로 언급)
   
   ## 4. 개발되지 않은 부분 (소스코드 확인 결과)
   소스코드를 확인했지만 구현되지 않은 기능/모듈을 나열:
   - ❌ **[기능/모듈명]**: [왜 필요한지, 어떤 파일에 있어야 하는지]
   - ❌ **[기능/모듈명]**: [구체적인 설명]
   - ... (누락된 기능을 구체적으로 언급)
   ...
   - "개발된 요소: [M]개"
   - "개발되지 않은 요소: [K]개"
   - "따라서 진행도는 [M/N * 100]%입니다."
   - 추가로 Task 완료율, 커밋 활동 등을 고려하여 최종 진행도 조정
...
- 소스코드를 직접 확인하여 구체적인 파일명, 함수명, 모듈명을 반드시 언급하세요.
- 진행도 계산은 논리적으로 수행하세요: (개발된 요소 수 / 필요한 요소 수) × 100
- 각 섹션을 모두 포함하여 작성하세요.
